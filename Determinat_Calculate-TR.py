...
def GetMatrix():
    ...
    # Programın kullanımı hakkında kullanıcıyı bilgilendirme
    print("Satırları yazmanız bitince \"exit\" yazın.")
    print("Satırlardaki değerleri birbirinden ayırmak için ','(virgül) kullanın.")
    ...
    while True:#Kullanıcı exit yazmadığı sürece döngüye kullanıcıdan matrixin her bir satırını almaya devam edecek olan döngü
        row = input(f"{counter}. satırı girin: ")
